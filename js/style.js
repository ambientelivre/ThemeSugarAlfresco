...
$(document).ready(function(){
    // Garante que nao sera mostrado o submenu ao mudar de pagina
    //$("span").removeClass("selected");
    // Açao padrao do SugarCrm
    $("ul.clickMenu").each(function(index, node){
            $(node).sugarActionMenu();
    });   
    
    /* Ao clicar fora, fecha os menus dropdown
     * O sugarcrm não faz isso por padrão, pelo menos no tema atual
     * */
    $('body').click(function(evt){    
        ...
        if($(evt.target).closest('#moduleList').length) {
            //alert("closest moduleList            
        return;
        }
        //Do processing of click event here for every element except with id menu_content
        if($(evt.target).closest('#subModuleList').length) {
...
function IKEADEBUG()
{
    // Preciso pegar a posicao top-left para renderizar o subModuleList no lugar correto
    var id_submenu; // id do submenu
    var element;
    var left; // posicao x inicial que o submenu ficará
    var bottom; // posicao y inicial que o submenu ficará
    ...
    moduleLinkMouseOver = function() 
        {
            ...
            for (var i = 0; i < moduleGroups.length; i++) { 
                if ( i == tabNum ) {
                    // O subgrupo que aparecerá na tela
                    moduleGroups[i].className = 'selected';  
                    //alert("id: "+moduleGroups[i].id);
                    id_submenu = moduleGroups[i].id;
                }
                else {
                    moduleGroups[i].className = '';
                    /* Se for clicar em outro moduleList, havera acumulo de menus, favor corrigir */
                    $(moduleGroups[i]).hide();
                }
            }
            ...
            for (var i = 0; i < groupList.length; i++) {
                ...
                if ( aElem.id == "grouptab_"+tabNum ) {
                    ...
                    var rect = aElem.getBoundingClientRect();
                    element = aElem;
                    left = rect.left;
                    bottom = rect.bottom;
                    //alert("x="+x+", y="+y);
			...
			if(menuHandle && parentMenu){
				updateSubmenuPosition(menuHandle , parentMenu);
                        ...
                        $( "#subModuleList span.selected").css("display", "block");
                        $(document.getElementById(id_submenu)).css("margin-left", left);                        
			////////////////////////////////////////////////////////////////////////////////////////
        };
    for (var i = 0; i < moduleLinks.length; i++) {
        /* controla como os menus aparecerao (mouseover ou onclick)*/
        moduleLinks[i].onclick = moduleLinkMouseOver;
    }
};

/* Css e html simples seria melhor do que isso para calcular posiçao do submenu em relaçao a esse metodo javascript
